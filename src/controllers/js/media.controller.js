    ...
    return async (req, res, next) => {
        ...
        if (contentType.includes('image/') || contentType.includes('application/octet-stream')) {
            try {
                ...
                const allowedExtensions = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'mp4', 'pdf', 'csv'];
                if (!allowedExtensions.includes(fileExt)) {
                    return res.status(415).json({
                        code: 'rest_invalid_file_type',
                        message: 'Tipo de arquivo não permitido por segurança.',
                        data: { status: 415 }
                    });
                }

                // Prevent executable extensions anywhere in name
                if (/\.(php|exe|sh|bat)$/i.test(filename)) {
                    return res.status(415).json({ error: 'Suspicious filename rejected.' });
                }
